...
- [ ] reactive-replay-harness — point-in-time counterfactual backtest engine: re-simulate a candidate config over a historical window by driving the landed reactive + survival cores + paper-fill sim, returning a per-period outcome record (the substrate `walkforward-tuning-loop`'s CPCV gate scores). Extracted from walkforward-tuning-loop 2026-05-30; data confirmed retrievable on Massive Advanced/Business (deep-research 2026-05-30). Dependencies: reactive-signal-model, survival-gate, broker-cfd-adapter, decision-trace-telemetry
- [ ] walkforward-tuning-loop — async after-market LLM batch: fit a trial set → **in-process CPCV replay** (via `reactive-replay-harness`) → DSR/PSR/PBO gate selects-best → autonomous promote (**amends §14.6** — operator 2026-05-30). **Owns its tuner-action audit (P11)** — falsifiable promotion hypothesis (P15), correlated to the model trace via shared keys. Dependencies: decision-trace-telemetry, execution-daemon, reactive-replay-harness
...
> Build order: **broker-cfd-adapter, reactive-signal-model, decision-trace-telemetry in parallel → survival-gate → {execution-daemon, reactive-replay-harness} → walkforward-tuning-loop → in-session-monitor.**

## Brief status (foundation-first, operator decision 2026-05-29)

**All eight specs now have briefs (2026-05-30)** — incl. `reactive-replay-harness`, extracted from `walkforward-tuning-loop` at its /kiro-validate-design (the point-in-time CPCV backtest engine; data feasibility confirmed against the Massive API by deep-research 2026-05-30, Advanced/Business tier). `broker-cfd-adapter`, `survival-gate` (Survive-first foundation), `reactive-signal-model` and `decision-trace-telemetry` (the two "Dependencies: none" inner-ring foundation specs — briefed 2026-05-29 once §11–§14 was committed; NOT gated by the §14.11 questions), plus `execution-daemon`, `walkforward-tuning-loop`, and `in-session-monitor` (all briefed 2026-05-30; see below). The §14.11 operator questions are **all resolved (2026-05-29):** #1 version-pinned lifecycle (first-principles target; moot for paper phase per §16.1 intraday-flat), #2 fully-autonomous promotion, #3 no trade-count floor → PSR/MinTRL, #4 no aggregate cost cap, #5 anchored/rolling adopt-and-validate, #6 promotion gate = DSR + PSR + PBO (research-confirmed; see `docs/research-walkforward-tuning-loop-2026-05-29.md`).

**Phases (2026-05-30):** `decision-trace-telemetry` in implementation (inner-ring tests); `broker-cfd-adapter` + `survival-gate` tasks-generated; `reactive-signal-model` + `execution-daemon` design-approved; `walkforward-tuning-loop` requirements-generated; `in-session-monitor` initialized (requirements pending). **`execution-daemon` briefed 2026-05-30** — Path C; full version-pinned lifecycle + atomic hot-swap IN scope (operator override of the §14.11 #1 "moot-for-paper" default); claims the previously-floating telemetry row-assembly + `run_id`/`walk_forward_window` injection seam. **`in-session-monitor` briefed + initialized 2026-05-30** — Path C own-spec (placement confirmed at discovery per §15); seams reconciled against the landed `execution-daemon` (command surface = 3 gated seams; monitor *selects*, daemon *hot-swaps*) and `walkforward-tuning-loop` (writes the P2 registry menu); the open **command-transport** seam (out-of-process monitor → non-MCP daemon) is flagged for design + a possible `execution-daemon` follow-up. **Migration numbers claimed:** 048 telemetry · 049/050 survival-gate · 051/052 execution-daemon · 053+ walkforward tuner-audit · ≥054 in-session-monitor audit (if a DB table).
