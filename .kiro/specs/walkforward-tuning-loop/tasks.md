...
- [ ] 1. Foundation: owned contract types + audit migration

- [ ] 1.1 Owned in-memory contract types (`types.py`) — the dependency-root barrier
...
- [ ] 1.2 (P) Migration 053 — `walkforward_tuner_audit` append-only table
...
- [ ] 2. Core compute leaves (pure; parallel after types)

- [ ] 2.1 (P) CPCV partition scheme (`cpcv.py`)
...
- [ ] 2.2 (P) Survival-net metric + calibration (`metric.py`)
...
- [ ] 2.3 (P) Deterministic promotion gate (`gate.py`) — the statistical crux
  ...
  - Observable: selects the highest survival-net config; **DSR rises with trial count**; **a degenerate trial set (N=1) ⇒ no-promote** (not a spurious pass); **PSR rises with negative skew/kurtosis**; **IS-Sharpe never changes the verdict**; insufficient-data ⇒ no-promote; §13 guard rejects an Edge/Return gain that lowers Survive/Preserve; identical inputs ⇒ identical verdict
  - _Requirements: 4.5, 5.1, 5.2, 5.3, 5.4, 5.5, 6.2, 6.3_
  - _Depends: 1.1_
  - _Boundary: gate_

- [ ] 2.4 (P) Trial-set assembly (`fit.py`)
...
- [ ] 2.5 (P) Firewall-bounded read + event drain (`read.py`)
...
- [ ] 3. Handoff, audit, and validator

- [ ] 3.1 (P) Version publish + IS-boundary advance (`publish.py`)
...
- [ ] 3.2 Tuner-action audit assembly + append-only write (`audit.py`)
...
- [ ] 3.3 HG validator for the audit envelope (`tuner_action_audit_shape.py` + registry)
...
- [ ] 4.1 Cycle orchestrator (`.claude/commands/walkforward-tune.md`)
...
- [ ] 4.2 E2E cycle test (Python leaf-wiring, stub harness) — the conservative decline path
...
- [ ] 4.3 Integration test (`integration_live`) — migration guard + publish round-trip
...
- venv: provision like RRH's `.venv-replay` — needs `httpx` (RRH `data_client` imports it at module load), `psycopg`, `numpy` (+ `scipy` if the gate uses `scipy.stats` for the normal CDF / skew-kurtosis terms — prefer a small in-module implementation to keep the inner ring lean), `pytest`, `python-dotenv`.
