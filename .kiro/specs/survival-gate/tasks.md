...
- [x] 3.1 Build the shared account-level margin-distance check
  ...
  - _Boundary: gate_
  - _Depends: 1.1, 1.2_
- [x] 3.2 Implement the order-admission veto walk
...
- **Minor hardening note (5.1 finding, NOT a defect; R2.5 holds):** `params.tighten_only` does not finiteness/type-validate override values the way `resolve` does. The never-loosen invariant always holds; an `inf` override on a higher-is-tighter field (e.g. `stop_out_level_pct`) is *accepted* and fails **toward maximum protection** (everything reads as breaching → always FLATTEN/reject — fail-safe, not a survival hole). Optional future hardening: make `tighten_only` finiteness-validate like `resolve`. The 5.1 tests assert the requirement-level *never-loosen* property, so they pass both today and a hardened build. (5.1)
- **Kill-switch-bypass fix (final `/kiro-review`, 3.2 remediation):** `_is_true_exit`'s net-reducing test was `order.volume <= held.volume` (no lower bound), so a **zero/negative-volume** opposite-side order — which net-reduces nothing — was misclassified as a true exit and short-circuited `admit` to ALLOW **past an engaged kill switch** (R9.1 violation). Fixed: the exit carve-out now requires `math.isfinite(order.volume) and 0.0 < order.volume <= held.volume`. A non-reducing order falls through to the full walk → REJECTED under kill-switch (fail-safe). Lesson: the exit carve-out's *defining property* is "positively net-reduces," not the operational shorthand `volume ≤ held`; the per-task 3.2 tests covered `volume==held` but never zero/negative. (3.2 remediation)
- **`admit` (3.2) seams for 4.1/4.2/5.2:** (a) **`grade_rank`/`_GRADE_RANK`/`HALT_NEW_RANK` are centralized in `src/survival/types.py`** (NONE=0,TIGHTEN=1,HALT_NEW=2,FLATTEN=3 — verified drift-free vs migration 049's trigger). Task 4.1's safe-mode escalation MUST reuse `grade_rank` (compare by rank, never string); `grade_rank` fails an unrecognized grade toward FLATTEN. (b) **`OrderEvaluation{additional_used_margin: Optional[float]=None, in_universe: bool=False, is_excluded: bool=True}`** (in `types.py`) is the single daemon-supplied context with **reject-leaning defaults** — `OrderEvaluation()` rejects every open. **Daemon-seam contract: `execution-daemon` MUST populate it** (revalidation-trigger-adjacent — flag in that spec). (c) Exit classified by EFFECT in `_is_true_exit` (gate.py): opposite-`direction` (LONG/SHORT) + `volume ≤ held` + exactly-one-held-in-symbol; `intent` ignored; fails-toward-open on unrecognized direction / multi-position. (3.2)
