...
- **`admit` signature — daemon-supplied `OrderEvaluation` context [cross-spec, IMPLEMENTED 2026-05-30]:** the design's narrative `admit(order, state, op_state, params, clock)` is realized with one additional typed input — `admit(order, state, op_state, params, clock, evaluation: OrderEvaluation)`. `OrderEvaluation{additional_used_margin: float|None, in_universe: bool, is_excluded: bool}` (in `src/survival/types.py`) carries the three values the pure core cannot derive (margin-from-volume / universe membership / screen-flag — all Phase-2/broker/screen knowledge). It has **reject-leaning defaults** (`OrderEvaluation()` → `additional_used_margin=None` rejects on `margin_distance`, `in_universe=False` rejects on `universe`, `is_excluded=True` rejects on `entry_exclusion` when enabled), so a missing/unknown screen fails toward not-adding. **`execution-daemon` MUST populate this context** when calling `admit` (it is the typed form of the "screen-predicate extraction" Phase-2 seam in the first bullet). Revalidation-trigger-adjacent: a change to `OrderEvaluation`'s shape or defaults re-fires the daemon integration.
