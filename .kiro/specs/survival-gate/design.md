...
- `db/migrations/0NN_survival_params_seed.sql` — **NEW** seed of the `survival.*` namespace (pattern: mig 038/039).
- `db/migrations/0NN_survival_gate_state_and_events.sql` — **NEW** `survival_gate_state` (monotonic op-state) + `survival_gate_events` (append-only; mig-003 trigger pattern).
...
- **HG-validator consistency [follow-up, not now]:** `reactive-signal-model`'s design also proposed an HG validator on a deterministic core — same "agent-rule applied to a non-agent" question. Worth a one-line follow-up pass on that spec; not a reopen here.
