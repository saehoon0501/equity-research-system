...
- Decision-substrate field change — `decision-trace-telemetry` (`decision_process_trace`); the substrate's `code_version`/`param_version` map to the landed `CorrelationKeys` typed columns (mig 048 / `schema.py`), so a rename either side is a cross-spec break.
...
The model owns no persistent storage. Its **output data contract** is `ReactiveDecision` (above); its **input parameter contract** is `ParamSnapshot` (by value). The `DecisionSubstrate` is the cross-spec payload that `decision-trace-telemetry` persists into `decision_process_trace`. **Substrate→row mapping (aligned to the LANDED contract — `src/reactive/telemetry/schema.py` `CorrelationKeys` + mig 048):** the model's `code_version` + `param_version` promote to the row's **typed correlation columns**; `feature_values` / `probability` / `effective_threshold` / `calibration` become the JSONB `trace` payload. This model's `param_version` **is** the telemetry `param_version` — same semantics (the consumed `ParamSnapshot` version), not a coincidental name-share; likewise `code_version`. The landed `CorrelationKeys` is **4 keys** (`run_id`, `code_version`, `param_version`, `walk_forward_window`); this pure model contributes only the **2 it owns** — `run_id` (run-context) and `walk_forward_window` (tuning-orchestration context) are **not** model inputs/outputs and are supplied by the daemon when it assembles the row (see Open Questions). Field names must stay aligned (revalidation trigger). `feature_values` carries the reused continuous components (reversion `rsi_14`/`drawdown`/`bollinger`/`ma_distance`, flow `composite_score_normalized`, tactical `bin`, `atr`) so a fire is reconstructable.
...
- **Correlation-key alignment with the LANDED telemetry contract [RESOLVED 2026-05-30].** `decision-trace-telemetry` shipped (mig 048 + `src/reactive/telemetry/schema.py`): `decision_process_trace` carries **4 typed correlation columns** — `run_id NOT NULL`, `code_version NOT NULL`, `param_version NOT NULL`, `walk_forward_window NULL`. The **daemon is the row-assembler** — it mints the client-side `trace_id` and pins `event_ts`, neither of which this pure model can supply, so the **2/2 split is forced, not chosen**: this model emits `code_version` + `param_version` (carried on `DecisionSubstrate`/`ParamSnapshot`) in its substrate; the daemon supplies the other two. **Forward-obligation [currently un-owned — flag for the future `execution-daemon` brief]:** `execution-daemon` MUST populate `run_id` + `walk_forward_window` on the `CorrelationKeys` (P3 — the daemon mints `run_id`/window) when it assembles the `DecisionTraceRow` from this model's substrate. The telemetry spec states only a generic "complete `CorrelationKeys`" precondition (it never assigns these two), and `execution-daemon` is un-briefed — so this obligation is floating; if lost, the first live write throws `run_id NOT NULL`.
