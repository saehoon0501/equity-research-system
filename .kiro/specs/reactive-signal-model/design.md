...
- **`DecisionSubstrate` field alignment** with `decision-trace-telemetry`'s `decision_process_trace` — confirm at that spec's design. The full correlation-key contract that table requires is **four** keys: `run_id` (NOT NULL), `code_version` (NOT NULL), `param_version` (NOT NULL), `walk_forward_window` (nullable). This model sources only `code_version` + `param_version` (carried on `DecisionSubstrate`/`ParamSnapshot`); `run_id` and `walk_forward_window` are **injected by `execution-daemon`** when it persists the trace (P3 — the daemon mints `run_id`). Stating only the two model-sourced keys here is insufficient for a trace write — the consumer must populate all four.
