...
- [x] 2.3 Derive the probability and expose the snapshot's calibration evidence
  ...
  - _Boundary: signal_model_
  - _Depends: 2.2_
- [x] 2.4 Emit the thresholded decision with sizing hint, non-final flag, and substrate
...
- 2.4: **design.md System-Flows mermaid — RECONCILED 2026-05-30.** It previously showed `decide` taking raw `ticker_bars`/`spy_close`/`rf` and history-gating itself, work the binding failure-ownership prose (line 179) assigns to `compute_features`. The mermaid + the "Key decisions" prose + the Architecture-Pattern note now reflect the **two-stage split**: `compute_features → FeatureSet | FeatureFailure` (owns history/ATR gating), then `decide(features, direction, …)` validates direction-first → maps `FeatureFailure`→HOLD → derives. When BOTH a `FeatureFailure` and an invalid direction occur, `decide` resolves **direction-first** (validate the caller's own argument before trusting other inputs) — line 179 is silent on the order; the choice is tested (`test_decide_invalid_direction_wins_over_feature_failure`) and docstring-documented. Abstain returns carry `probability=0.0` as a non-derived sentinel (the load-bearing discriminator is `reason != None`); `direction_in` echoes the raw invalid value on the invalid-direction path while `decision` stays in-vocab.
