...
## Calibration-substrate correction (2026-05-30 — surfaced at /kiro-impl task 2.1)

The /kiro-impl run (workflow) **correctly halted at task 2.1** (STOP_FOR_HUMAN) rather than fabricating a label. Verified against ground truth (live DB `\d counterfactual_ledger`, mig 048 SQL, `signal_model.py`):

- **Real gap:** the diagnostic's Brier/reliability/ECE need a **per-reactive-decision realized *directional* label** (`signal_model.py:130` — `P(caller direction is the correct side)`). That label is **not on `decision_process_trace`** (decision rows carry softmax `probability` but no realized-direction outcome; fill rows carry fill-quality only — no exit/round-trip pairing) and is **owned by `walkforward-tuning-loop`** (`signal_model.py:181` — the reactive layer "NEVER computes Brier/reliability over realized outcomes"), which has not landed.
- **Correction to Issue-1's baseline decision:** the original design said v0.1 computes the baseline from `counterfactual_ledger`. **mig 048 *did* add the 4 version keys to that table** (so the debug agent's "lacks the 4-key contract" claim was wrong), BUT it is the **slow-layer outer-ring eval ledger** (`summary_code` ∈ {BUY,HOLD,TRIM,SELL} over 90d/1y/3y/5y, sector-ETF excess return, per *research* decision) — a **different grain/vocabulary**, not commensurable with reactive per-decision directional calibration. Computing the baseline there would silently compare two different things.
- **Resolution (operator-chosen: "fix design + label seam, build all"):** the diagnostic consumes an injected **`RealizedLabelSource` seam** (Protocol in `diagnostic.py`); the per-version baseline uses the **same seam**, not `counterfactual_ledger`. v0.1 injects a source yielding no reactive labels → diagnostic **INSUFFICIENT** (correctly blind), wiring into the existing R2.4 insufficiency path. Structure + inner-ring tests build now (synthetic source drives the calibration path); the live signal activates when the reactive realized-directional-label source lands (**Revalidation Trigger**). design.md §209/§211/§279 + the revalidation-triggers list corrected; tasks.md 2.1 updated.
