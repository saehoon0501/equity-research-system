...
- **The validated-version "menu" contract — RESOLVED 2026-05-30** (walkforward-tuning-loop design landed). The menu is the **P2 `parameters` machinery** (no bespoke registry): walkforward `publish` writes validated values into the `parameters` rows so `parameters_active` resolves to them. `intervene`'s `SELECT_SAFER_CONFIG` reads the validated-version history from `parameters` (with `parameters_active` as the current); the daemon validates the chosen `param_version` is a member + not survival-loosening. Re-fires only on a future P2 shape change.
- **Reactive realized-directional-label source (`RealizedLabelSource` seam) + per-version `in_sample_baseline`** — CORRECTED 2026-05-30 (calibration-substrate grain fix, surfaced at task 2.1 impl): the diagnostic's Brier/reliability/ECE need a per-decision realized *directional* label that is **not on `decision_process_trace`** and is **owned by `walkforward-tuning-loop`/a future reactive realized-outcome surface (not landed)**; `counterfactual_ledger` is the *slow-layer* eval ledger (wrong grain/vocabulary) and is **not** the substrate. v0.1 injects a label source that yields no reactive labels → diagnostic INSUFFICIENT (correctly blind). **Re-fires (diagnostic + baseline re-check) when the reactive realized-directional-label source lands** — wire it (and optionally the tuner's published per-version baseline) into the seam then.
- **`execution-daemon` order-path revisions (Rev 2.2–2.4.1, APPROVED) do not touch this spec** — they add `candidate`/`order_builder`/`PinnedParams.reactive_snapshot`/`ProposedOrder` (the Edge order path); the command-intake transport, the three command types, `issued_by`/`applied_at`, the `select-validated-config`→hot-swap, and mig 051/052 are unchanged. Re-fires only if a future daemon revision changes the **command-intake** contract specifically.
...
### Leaf — `diagnostic`
**Responsibilities & Constraints**: read recent trace via `query_trace` **filtered to the active `(code_version, param_version)`** (Issue 1 — calibration is meaningless across a hot-swap; the keys + index exist in mig 048, and `query_trace` already filters on them), extract per decision-row the softmax `probability` **and the derived survival-proximity fields** (`liq_proximity`, `stop_out`, `gate_link`), and pair each decision's `probability` with its **realized directional label obtained from an injected `RealizedLabelSource` seam** (a `Protocol` defined in `diagnostic.py`). *Calibration-substrate correction (2026-05-30):* the reactive per-decision realized outcome — *was `P(caller direction)` the correct side?* — is **NOT carried on `decision_process_trace`** (decision rows carry the softmax `probability` but no realized-direction outcome; fill rows carry fill-quality only — `expected_price`/`actual_fill_price`/`slippage` — with no exit/round-trip pairing) and is **owned by `walkforward-tuning-loop`/a future reactive realized-outcome surface, not yet landed** (`signal_model.py` R7.4: the reactive layer "NEVER computes Brier/reliability over realized outcomes — owned by walkforward-tuning-loop"). For **v0.1 the orchestrator injects a source that yields no reactive labels**, so the diagnostic returns INSUFFICIENT — correctly blind on calibration drift until that surface lands. Compute calibration over the version-scoped window (`brier_score`, `reliability_diagram`, `expected_calibration_error`) **with the block-bootstrap CI**, and compare against **that version's** `in_sample_baseline` in `MonitorParams`. A window that would cross a hot-swap is **restricted to the current version** (the prior version's rows are not mixed in). Return a `DriftDiagnostic` (per-metric observed value + CI + survival-proximity summary, tagged with the analyzed version keys) or an explicit insufficient-data result when `N < min_observations` (2.4) — **note: immediately after a hot-swap the current-version window is below `min_observations`, so the monitor is correctly INSUFFICIENT/blind until it refills** (expected, not a defect). Pure read; no write; reuses landed metrics (no reimplementation).
```python
class RealizedLabelSource(Protocol):
    # Injected seam. For a version-scoped window of decision rows, yield per-decision
    # (probability, realized_binary_label) pairs. v0.1 source yields NONE (the reactive
    # realized-directional-outcome surface is unlanded -> owned by walkforward-tuning-loop).
    def labels_for(self, rows: list[dict]) -> list[tuple[float, bool]]: ...

def compute_drift(filters: dict, params: MonitorParams, label_source: RealizedLabelSource, conn=None) -> DriftDiagnostic: ...
# DriftDiagnostic carries: per-metric (observed, ci_low, ci_high), baseline, window_n, in_survival_band: bool, sufficient: bool.
# Postconditions: sufficient is False when window_n < params.min_observations OR label_source yields no realized labels
#   (v0.1: no reactive realized-outcome surface -> INSUFFICIENT). Unit tests inject a synthetic source to drive the calibration path.
```
...
**Baseline-ownership decision (corrected 2026-05-30 — calibration-substrate grain fix):** `in_sample_baseline` is **per `(code_version, param_version)`**, computed over the version's in-sample window **from the same `RealizedLabelSource` seam the live diagnostic uses** — NOT from `counterfactual_ledger`. *Correction:* although mig 048 bolted `code_version`/`param_version`/`walk_forward_window` onto `counterfactual_ledger`, that table is the **slow-layer outer-ring eval ledger** (P14 outer-ring: `summary_code` ∈ {BUY,HOLD,TRIM,SELL} over 90d/1y/3y/5y, sector-ETF excess return, per *research* decision) — a different **grain and outcome vocabulary**, not commensurable with the reactive per-decision directional calibration; computing the baseline there would silently compare two different things. The reactive realized-directional label is **owned by `walkforward-tuning-loop`/a future reactive realized-outcome surface, not yet landed**, so for **v0.1 both the live diagnostic and the baseline are INSUFFICIENT until that source lands** (the monitor's loop, audit, validator, and intervention plumbing are built + inner-ring tested against a synthetic `RealizedLabelSource`; the live calibration signal activates when the real source is wired). **Revalidation Trigger:** wire the real reactive per-decision realized-directional-label source (and optionally the tuner's *published* per-version baseline) when `walkforward-tuning-loop` lands it.
