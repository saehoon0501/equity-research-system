...
- **Per-version `in_sample_baseline` ownership** — v0.1 has the monitor compute the baseline itself (Issue 1 decision); **this stands** — walkforward computes calibration only for its own gate + tuner-action audit and does **not** publish a reusable per-version baseline. The tuner's per-version calibration lives in its tuner-action audit (joinable via the 4 keys) as a *potential future* baseline source: switch the monitor to read it if/when that coupling is wanted, to avoid two components deriving "the validated baseline" differently.
