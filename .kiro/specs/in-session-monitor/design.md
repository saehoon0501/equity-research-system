...
### Leaf — `diagnostic`
**Responsibilities & Constraints**: read recent trace via `query_trace`, extract per decision-row the softmax `probability` + realized outcome **and the derived survival-proximity fields** (`liq_proximity`, `stop_out`, `gate_link`), compute calibration over a window (`brier_score`, `reliability_diagram`, `expected_calibration_error`) **with the block-bootstrap CI**, and compare against the **pinned in-sample baseline** carried in `MonitorParams`. Return a `DriftDiagnostic` (per-metric observed value + CI + the window's survival-proximity summary) or an explicit insufficient-data result when `N < min_observations` (2.4). Pure read; no write; reuses landed metrics (no reimplementation).
```python
...
### Leaf — `audit`
**Responsibilities & Constraints**: build the `InterventionAudit` (triggering diagnostic + falsifiable rationale + the 4 keys + optional `command_ref`) and persist it as an envelope. **The 4 correlation keys are the daemon `execution_daemon_epoch` keys read from the analyzed trace rows** (Rev 2.1), so the audit joins the model trace + ledger (7.3) — these are distinct from the monitor's own orchestration run_id used only to name the envelope. Owns the **why** only — the daemon emits the command **what** as a `command`/`safe_mode`/`kill_switch` event; the two join via the 4 keys (7.4). `conn=None` dry-run. Append-only by convention (one per tick).
```python
...
| command_ref | str \| null | Phase 2: the daemon channel row id (the *what*); null in Phase 1 |
...
- **v0.1 paper**: a documented permissive DB grant is accepted. **Before any live real-money cutover (8.4 gate), the commander runs under a dedicated DB role/grant** restricting INSERT on `execution_daemon_command_intake` to the monitor + operator identities.
...
- **diagnostic (1.2, 2.1, 2.4)**: calibration computed over a window from synthetic trace rows; `sufficient=False` below `min_observations`.
