...
- **Build-time validation (P12/P14):** before the adapter is trusted, one authenticated round-trip — read `symbols/detail` for a known ticker, read `assets` + `mt5-account`, and (in paper) exercise the order→poll→close flow — confirms the field names and the async/position-id lifecycle against a live account.
  - **Vehicle (operator decision 2026-05-29 — reference + validation only):** use **`gate/gate-local-mcp`** (npm `gate-mcp` ≥0.19.0; TypeScript stdio MCP exposing Gate APIv4, MIT) in **read-only mode** (`GATE_READONLY=true`, `--modules=tradfi`) for the authenticated readout round-trip — it is a working reference implementation of the `/tradfi/*` surface (categories, symbols, MT5 account, assets, orders, positions). It grounds the live field names, the async order→poll→close lifecycle, `close_type` 1/2 semantics, and the `X-Gate-RateLimit-*` headers (G24), and serves as a real APIv4 `/tradfi` signing reference (informs G23 — earlier "official SDK doesn't cover TradFi" is now softened). **It does NOT replace `broker-cfd-adapter`:** it has no in-adapter paper simulation (Req 8), no enforcement layer (Req 4/5/6), no kill-switch/survival-gate live-send gating (Req 8), no BUY/HOLD/TRIM/SELL P9 mapping (Req 1), no double-send guard (Req 7.4), and is MCP-only (the daemon needs in-process leaf functions, not an MCP). The broader hosted sibling `gate/gate-mcp` (`api.gatemcp.ai/mcp/exchange`, OAuth2, 400+ tools incl. TradFi) is out of posture for the self-hosted paper phase.
