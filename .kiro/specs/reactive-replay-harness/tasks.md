...
- [x] 4.2 (P) Massive `integration_live` smoke — the live-probe gate _(test complete; live probe RAN 2026-05-30 and reported the precise gap per the observable — see Implementation Notes 4.2: current Massive key NOT_AUTHORIZED for historical market data; operator plan-upgrade to Advanced/Business is a separate deployment prerequisite before live replay)_
...
- **4.2 — DONE 2026-05-30 (test complete; live probe ran).** The `integration_live` test is implemented, auto-skips by default, and on explicit `-m integration_live` it executed against the live key in `.env` and reported the precise gap (4.2 observable: "passes OR reports the precise gap"). **Live-data feasibility, however, is NOT met on the current plan** — see Impl Note 4.2: 3/8 probes pass (splits/dividends/market-holidays reference endpoints entitled), 5/8 FAIL `NOT_AUTHORIZED "Your plan doesn't include this data timeframe"` (SPY daily, S&P500 daily, /v3/trades, /v3/quotes, delisted OHLC). **Operator action: upgrade Massive to Advanced/Business before any live replay** — the code is correct (client raises `DataFetchError(auth)`, test reports faithfully); this is a deployment prerequisite, not a code defect.
...
- 4.1/4.3: independent reviewer (kiro-review) APPROVED both; tight `1e-6` fidelity tolerance bite verified (good seed -> pass, mis-seed -> fail), and both not-evaluable branches (absent baseline + sparse lone-leg) asserted distinct from `fail` (R7.3).
- 4.1 FULL-SUITE FIX (caught by `/kiro-validate-impl`, 2026-05-30): `test_full_engine_runs_with_no_db_driver_imported` passed SCOPED (`pytest tests/unit/reactive/replay/`) but FAILED under the full suite — its `assert "psycopg" not in sys.modules` is global-state-fragile: collecting `tests/integration/conftest.py` imports psycopg at module load, so the absolute-absence check trips even though `replay_candidate` never reaches a DB path. Fixed by booby-trapping the DB seam `harness._read_champion_config_p2` to raise if reached (call-time module-global lookup at harness.py:188-192, so the monkeypatch bites) + a DELTA sys.modules check — mirrors the reviewer-verified network-seam proof in `test_injected_port_short_circuits_prod_data_client_construction`. The reviewer had flagged this exact order-fragility as a Suggestion; the cross-task gate actualized it. LESSON: inner-ring isolation tests must assert behaviorally (seam booby-trap / delta), never on absolute global `sys.modules` membership for a module legitimately imported by sibling test infra.
- CLOSE-OUT 2026-05-30: all 16 sub-tasks done; full feature suite green in `.venv-replay` (174 unit passed + 8 `integration_live` skipped, 0 failed) under BOTH the scoped and full `tests/unit + tests/integration` orderings; public entry imports clean. Feature gate `/kiro-validate-impl` run at close-out → GO after the 4.1 full-suite fix.
