...
sequenceDiagram
    ...
    alt order contemplated
        ...
        alt ALLOW and actionable and within survival size
            ...
            Note over L: record binding_constraint / declined; resize+re-admit if advisory
        end
    end
...
```
Key decisions: the **command intake is polled first each cycle** so a just-issued kill-switch/safe-mode is observed before any admit (op-state-freshness extended to the out-of-process transport); `assess` runs **every tick**; the op-state transition is **persisted before** any directive/admit (persist-then-act); a REJECT with an advisory max triggers a single resize-and-re-admit.
...
- **`orchestrator` (2.x, 3.x, 5.1, 7.1, 10.x):** admit-before-decide-before-submit; REJECT kills the order + records the constraint; size-breach → resize ≤ advisory + single re-admit; HOLD → declined trace; never-upsize; persist-then-act.
