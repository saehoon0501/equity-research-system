...
├── candidate.py         # each eval: fetch fast-clock data (ticker bars + SPY + rf_yield) from the market feed; compute_features via the reused overlay cores (tactical/flow/reversion + atr); map tactical bin -> direction (positive->LONG, negative->SHORT, neutral/unavailable->None) (§12.3), reading the bin from features.raw['tactical_bin'] NOT trend_vote (NB-1: trend_vote folds unavailable into neutral); surface reference_price = last close (CN-4); -> Candidate{features, direction, reference_price} for decide + order_builder. Returns None (no new exposure) on a non-directional bin (Req 12.5) OR missing/insufficient data (Req 12.4)
...
├── test_candidate.py    # feature computation over synthetic fetched arrays; bin->direction map (positive->LONG, negative->SHORT) read from raw['tactical_bin'] not trend_vote (NB-1: assert unavailable and neutral are distinguishable, both->None but attributable to 12.4 vs 12.5); reference_price = last close surfaced on the Candidate (CN-4); missing/insufficient data -> None/no order (Req 12.4) (mocked feed; reused overlay cores)
...
**Responsibilities & Constraints**: each evaluation, **fetch the fast-clock market data** the feature computation needs (the ticker's recent bars, the SPY benchmark series, the risk-free yield) from the market feed (12.1); **compute the `FeatureSet`** via `reactive.features.compute_features` over the fetched arrays — the daemon never passes raw data to `decide` (12.2); **map the `tactical-overlay` relative-strength bin to a candidate `direction`** (§12.3 — explicitly *not* from fundamentals or the slow-layer thesis; the slow-layer veto is the entry-exclusion stage housed inside `survival-gate`, §12.6). **The bin is read from `FeatureSet.raw["tactical_bin"]`** — the verbatim 4-valued string `compute_features` surfaces (`src/reactive/features.py:190`) — **never from `trend_vote`**, which collapses `unavailable`→`0.0`==`neutral` (the `_TACTICAL_VOTE` map omits `unavailable`, `features.py:83`; reading `trend_vote` would silently fold the two non-directional bins together, breaking the Req 12.5/12.4 distinction — NB-1). The tactical core (`src/overlays/tactical/bin_classifier.py`) emits a **4-valued bin**, mapped:
...
A **non-directional** bin (`neutral`/`unavailable`) is the *absence of an edge*, not a data error — it returns `None` (Req 12.5), as does missing/insufficient market data (Req 12.4). The candidate records **which** non-directional bin it saw (`neutral` vs `unavailable`) so the skip stays attributable to 12.5 (no edge) vs 12.4 (bad data) downstream, even though both halt new exposure. A present-but-**degenerate** risk-free rate (`rf_degenerate=True`, rf < 0.5%) still yields a directional bin — the relative leg resolves — so v0.1 does **not** suppress direction on it. On a directional bin, **surface the reference price** (`reference_price = ticker_closes[-1]`, the last close — `compute_features` computes it then drops it, so the candidate carries it explicitly, CN-4) and hand a `Candidate{features, direction, reference_price}` to the orchestrator for `decide` (12.4).
