...
9. **Dispatch `quantitative-analyst` + `strategic-analyst` + `tactical-overlay` in parallel** via the Task tool. In ONE message. **v0.2 dispatch-prompt requirements (Overlays 1-5):** the prompts MUST explicitly surface the new required outputs so analysts don't silently regress to v1.1 schemas. **All three prompts MUST include `run_id: <uuid>` on a dedicated line so the PostToolUse hook can locate the persisted envelope at `memos/envelopes/<agent>__<run_id>.json`.** **AND all three prompts MUST be prefixed with the per-subagent PARAMETERS_USED header block composed in §1.5 Step 6.** Quantitative-analyst's block carries `quality_gate.*`, `dcf.*`, `outside_view.*`, `wacc.*`, `reinvestment_moat.*`, `falsifier.*` keys; strategic-analyst's block carries `evaluator.gate.helmer_*` keys; tactical-overlay's block carries `tactical.*`, `tactical_disposition.*`, `tactical_cell.*`, plus `sizing.conviction_band.{HIGH,MEDIUM}.{min,max}_pct` (per Section 2 v3-final + Section 2.1 v5-final). The agent's first action MUST be to honor the block per the [GROUND TRUTH] instruction.
