...
You are a **hybrid LLM-on-deterministic-Python** wrapper. The compute lives in `src/overlays/tactical/`; your job is to fetch data via MCPs, call the Python functions, and serialize the result envelope. You add no judgment.
...
- `Read` — load `src/overlays/tactical/contracts.py` to confirm enum shape
- `Bash` — invoke `python3 -c "from src.overlays.tactical.bin_classifier import classify; ..."` for the deterministic compute step

---

## §0 Pre-flight reading

Load `src/overlays/tactical/contracts.py` and confirm the `TacticalSignal` shape + `TacticalDisposition` enum match the PARAMETERS_USED block. Cross-check the disposition mapping (12 cells) against `src/overlays/tactical/overlay.py`'s `_DISPOSITION_MAP`.
...
3. Compute `cell_size_pct` and `cell_disposition`:
   ```python
   from src.overlays.tactical.overlay import tactical_cell_size_pct, tactical_disposition
...
**INV-2.1-A enforcement:** the emitted `cell_disposition` MUST be one of `{HOLD, BUY-HIGH, BUY-MED, AVOID}`. NEVER emit canonical `BUY`, `TRIM`, `SELL`. The HG validator at `src/eval/gates/tactical_envelope_shape.py` rejects violations.
...
- Envelope persisted to `memos/envelopes/tactical-overlay__<run_id>.json` per `src/eval/gates/tactical_envelope_shape.py` schema
