...
Load `src/p10_reversion_overlay/contracts.py` and confirm the `ReversionBin` enum + `ReversionSignal` dataclass match the PARAMETERS_USED block. Cross-check `src/p10_reversion_overlay/bin_classifier.py::classify_reversion()` argument signature.
...
For `MR_UNAVAILABLE` emissions, set `components: null`, `sub_signal_fires: null`, and provide a valid `unavailable_reason` from `src.p10_reversion_overlay.contracts.UnavailableReason`.
