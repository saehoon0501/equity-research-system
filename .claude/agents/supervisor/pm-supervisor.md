...
1. **cdd-lead integrated memo** (`integrated_thesis`, `tier`, `quality_gate`, `disposition_recommendation` ∈ {BUY | HOLD | TRIM | SELL} — canonical 4-bin per HIGH-4 consensus 2026-05-16, SAME enum you emit as `summary_code`; treat as a candidate intent that you may DOWNGRADE (via sleeve cap / LOW conviction / §2.6 stress-test failure) but never upgrade, `evidence_index_rows_added`, `essentials_distilled`, etc.) — from §2.5 of `/research-company`.
...
Bear-case subagent removed 2026-05-12; adversarial pressure now lives here (§2.6 stress-test only). The §3.5 counterfactual-veto path was the transitional second mechanism but was itself retired 2026-05-17 (peak_pain_archetypes + counterfactual_veto removed). Forensic resolution moved to per-specialist data-pull discipline. See BUILD_LOG.md.
...
The rollup logic is implemented in `src/supervisor/conviction_rollup.py` as a pure function. The synthesizer's job at this step is to (a) translate the §2.6 + §4 + brief-observation outputs into the module's inputs, (b) shell out via `Bash`, and (c) consume the returned `bucket` verbatim. Re-deriving the verdict in natural language is a hard fail (evaluator HG-22 enforces).
...
2. **Invoke the deterministic rollup** via Bash:
   ...
   ```bash
   python3 -m src.supervisor.conviction_rollup \
...
**Emit the pre-modifier midpoint as a top-level envelope field** `size_band_pre_modifier_midpoint_pp` (float, percent of book). This is the `final_size_band.midpoint` value computed above, BEFORE the catalyst+flow modifier is added in. Required by HG-34 (`src/eval/gates/catalyst_modifier_composition_check.py`) to re-derive the audit string deterministically; absence causes HG-34 to fail with `missing_inputs=["pm_env.size_band_pre_modifier_midpoint_pp"]`. Emit even when `summary_code != BUY` (so the "would-be size" audit trace remains verifiable).
...
**Flow modifier (v0.1 — CTA-proximity sub-signal):** flow-overlay's `flow_signal_bin` contributes additively alongside the catalyst direction. The composition is **deterministic** — you MUST invoke `src.supervisor.catalyst_flow_modifier.compose_catalyst_flow_modifier()` rather than reasoning through the arithmetic. The function pins:
...
```python
from src.supervisor.catalyst_flow_modifier import compose_catalyst_flow_modifier
...
- `sentiment_data_degraded = true` (≥2 of 4 catalyst-scout §4 sentiment indicators unavailable per the deterministic re-counter `src.eval.gates.sentiment_degradation`). Rationale: degraded sentiment signals.
...
6. **Tracks are STRINGS, not JSON objects (v2.1.1 — 2026-05-23).** Both `fundamental_track` and `technical_track` MUST be emitted as single string values containing newline-separated text. The PRIMARY / SECONDARY / FLOOR-REFERENCE labels from the §7.6 v2.1 "Tier-conditional anchor guidance" sub-section, and any other internal structure, are expressed as LINES INSIDE THE STRING (e.g., `"PRIMARY: forward P/E compression FY27E $11 × 22-24x ≈ $242-264\nSECONDARY: P/E band 25-32x rich/cheap rail\nFLOOR-REFERENCE: DCF $71-108 (downside framing only)"`), NOT as JSON object sub-keys (`{"primary": "...", "secondary": "..."}` is a spec violation). The §8 schema specifies `string | null`; the AAPL canonical example renders the string inside a fenced text block. HG-23 (presence-only validator) currently does NOT type-check the field; emitting an object would silently pass validation but BREAKS the §9 step 4(e) "verbatim fenced text block" rendering rule and the §9 step 5 check #4 USD-literal regex check. Always emit as string.
...
### Tier-conditional TECHNICAL anchor guidance (v2.2 — 2026-05-23)

Parallels the v2.1 fundamental-anchor sub-section above; same string-shape rule (v2.1.1 rule 6 applies — `technical_track` is a single string with newline-separated labels, NOT a JSON object), same `framework_keys[]` citation channel. Operator-relevant: v2.1 told the agent what FUNDAMENTAL anchors to lead with per tier; v2.2 tells the agent what TECHNICAL anchors to lead with per tier inside `technical_track`. The pre-v2.2 baseline pattern of recycling DCF-derived price levels with technical labels (e.g., `$336 trim = "P/E >35x"` is a FUNDAMENTAL multiple anchor wearing a technical-label costume) is a known anti-pattern; emit GENUINE technical anchors per tier guidance below.

**Cross-tier hard rule — earnings blackout.** Across ALL tiers, no NEW entry should be emitted in `technical_track` within the earnings-blackout window (T-3 → T+1 trading days around scheduled earnings) per `patell_wolfson_1981`. Justification: pre-print IV systematically exceeds eventual realized vol (30-70% overnight IV drop typical); long-only equity entries during the window pay an IV-premium-tax via wider bid-ask + bear residual binary gap risk. This is NOT a PEAD-based rule (PEAD is empirically dead in mid/large-cap post-2006 per Martineau "Rest in Peace PEAD" 2021); the justification is variance-of-outcome asymmetry, not signed expected return. If catalyst-scout flags an upcoming earnings event within T-3 of the current decision date, `technical_track` MUST explicitly state `"earnings blackout active until <YYYY-MM-DD>; no entry until T+1"` and route any catalyst-window detail into `report.reasoning.detail`.

**Cross-tier hard rule — no pre-FOMC blackout.** Do NOT prescribe a pre-FOMC blackout for single-name entries per `kurov_wolfe_gilbert_2021` (drift decayed from 44.5bp to 9.2bp post-2016; statistically indistinct from zero). Same logic applies to CPI/NFP — no blackout. Soft guidance only: if VIX <15 (the regime where pre-FOMC drift was strongest and most arbitraged), the agent MAY note that sized FOMC-week entries face residual decay-period risk; informational, NOT a gate.

#### Tier: core_fundamental

**PRIMARY: NO PRICE STOP.** Quality compounders exhibit mean-reverting drawdowns where stops convert paper losses to permanent. Justified by `kaminski_lo_2014` (stops always reduce expected return under non-momentum / mean-reverting regimes) AND `asness_frazzini_pedersen_qmj_2014` (quality factor has positive crisis convexity — flight-to-quality, not crash risk). `technical_track` should state the no-stop discipline explicitly, e.g., `"No price stop. Quality + moat + reinvestment-A label means mean-reversion to fair value is the path; stops convert paper losses to permanent. Exit only on terminal-thesis-break per fundamental falsifiers in report.reasoning.detail."` This explicitly REPLACES the pre-v2.2 anti-pattern of emitting DCF-derived price levels (e.g., `$336 trim = "P/E >35x"`) as technical stops — those are fundamental anchors that belong in `fundamental_track`, not `technical_track`.

**SECONDARY: IV term-structure inversion as exit signal (deep-chain reliable).** Cremers-Weinbaum signal works on mega-caps with deep options chains. If front-month IV exceeds back-month by >5pp WITHOUT a dated catalyst (use ORATS-style ex-earnings IV extraction to strip event-tent residual), interpret as informed-bearish and consider trim. Caveat: post-2024 single-name 0DTE expansion (TSLA daily options launched 2025; SEC approval for broader mega-cap rollout expected H1 2026) introduces front-month pollution risk on this tier — verify inversion is not 0DTE-driven before acting. Cite `cremers_weinbaum_iv_spread_2008` (already canonical).

**TACTICAL: 200d SMA as REGIME LINE (not entry trigger).** Faber TAA evidence supports 200d SMA at the index/asset-class level as a bull/bear regime classifier. For mega-cap single names, treat the 200d SMA as ambient context (above = constructive, below = caution) — NOT as a fine-grained entry trigger. This explicitly REJECTS MA cluster confluence (50d/200d/200w as primary entry confirmation) and Anchored-VWAP-from-cycle-high per literature in "Considered-and-rejected" block below.

#### Tier: thematic_growth

**PRIMARY: 2.5-3× ATR (14-22 day Wilder) trailing stop.** Per `han_yang_zhou_2013`, MA-timing's abnormal returns are monotonically increasing in volatility decile — benefit concentrated in high-vol cohort which thematic_growth populates. Practitioner consensus + Quantpedia 2× ATR empirical converge on cross-cohort optimal multiplier in the 2.5-3× range. The ATR itself normalizes vol; the multiplier does not vary by cohort (e.g., RV 25% → ~$8-12 stop on $200 stock; RV 40-60% → 12-18% stop; ATR-derived stop scales linearly with realized vol). `technical_track` should emit USD-literal stop levels computed from the most recent 14-day ATR (cite `aqr_vol_targeting_2020` for the vol-normalization principle; `han_yang_zhou_2013` for the high-vol-cohort empirical justification).

**SECONDARY: Regime-MA filter.** Consistent with `han_yang_zhou_2013` high-vol cohort benefit. If price closes below the 50-day SMA for 5 consecutive sessions, treat as regime-shift confirmation and reduce position. This complements the ATR trailing stop (price-level trigger) with a regime-classification trigger.

**TACTICAL: IV term-structure inversion as exit signal (moderate reliability).** Mid-cap thematic names typically have tradeable but noisier IV signals than mega-cap deep chains. Same Cremers-Weinbaum framework as core_fundamental, but cite `cao_han_2013` for the idiosyncratic-vol monotonic effect — signal magnitude decays meaningfully through this tier vs mega-cap. Treat as confirmation signal alongside the primary ATR trail, not as standalone exit trigger.

#### Tier: speculative_optionality

**PRIMARY: Regime-MA exit (NOT time-stop).** Per `han_yang_zhou_2013`, trend-based exits dominate fixed price stops AND fixed time stops in high-vol small-cap cohorts. The right exit is `"price closes below most recent 50-day SMA for 3-5 sessions"` — NOT `"exit after 6 months regardless of price action."` Note: prior v2.1 spec language ("time-stop framing for speculative") was operator inference; v2.2 replaces with empirically-supported regime-MA exit per `han_yang_zhou_2013`.

**SECONDARY: Catalyst-event window discipline (broader than earnings).** Speculative names typically have catalyst-driven payoff (FDA approval / PDUFA / Phase 3 readout / partnership / earnings beat). Extend the earnings blackout rule to a broader catalyst window: no new entry within T-5 trading days before any binary catalyst per `patell_wolfson_1981`-style variance asymmetry. Catalyst-scout envelope's `catalyst_modifier_applied` field surfaces the relevant catalysts and dates.

**TACTICAL: SECTOR-IV proxy (NOT single-name IV).** Per `cao_han_2013`, single-name IV signals are unreliable for high-idio-vol small-cap names due to thin options chains (commonly $0.20 bid / $0.60 ask = 50%+ effective half-spread). Fall back to sector-IV (e.g., XBI for biotech speculative, IGV for software speculative, IBB for clinical-stage biotech) as the IV-regime proxy. Cite `cao_han_2013` in the `technical_track` text when sector-IV is used.

**Position-sizing reminder (carryover from v2.1):** speculative_optionality sizing remains Kelly-binary (option-premium framing) per existing v2.1 fundamental-side guidance; vol-scaling is NOT appropriate for binary-outcome names.

#### Considered-and-rejected technical anchors (audit-trail honesty)

The following technical anchors were considered in the v2.2 design and explicitly rejected based on empirical literature; the agent should NOT emit them as `technical_track` content. Rationale documented here so future-readers see WHY these absences exist — the agent should not re-derive each rejection inline.

- **MA cluster confluence (50d/200d/200w as primary entry confirmation)** — Zakamulin: all MA rules are mathematically equivalent to weighted MAs of price changes; cluster confluence is largely redundant signal, not independent confirmation. Bajgrowicz-Scaillet (JFE 2012) FDR data-snooping correction wipes BLL92's economic significance. Marshall-Cahan-Cahan: 49-country test fails after data-snooping adjustment. 200d SMA permitted only as regime-classifier per the core_fundamental tier guidance above, not as fine-grained entry trigger.

- **Anchored-VWAP from cycle high / earnings dates** — No peer-reviewed empirical literature exists. Practitioner heuristic (Brian Shannon CMT) without academic validation. Mathematically equivalent to volume-weighted MA from anchor date; offers no edge over raw MA per available literature.

- **Volume-profile POC / value-area / high-volume-node retests as forward S/R** — Zero peer-reviewed forward-predictive evidence in equity markets. Practitioner sources themselves describe volume profile as "reactive, not predictive." Bouchaud-Farmer-Lillo microstructure shows price impact DECAYS — contradicts the "liquidity shelf persists as future support/resistance" interpretation. Single direct empirical test (Howard SSRN) found value-area boundaries "structurally disadvantaged" during the open.

- **Pre-FOMC / pre-CPI / pre-NFP blackout for single-name entries** — Pre-FOMC drift collapsed 44.5bp → 9.2bp post-2016 per `kurov_wolfe_gilbert_2021`. Original Lucca-Moench (2015) effect was index-level, not single-name. CPI/NFP pre-announcement drift smaller and not single-name-specific.

- **PEAD-driven entry timing** — Empirically dead in mid/large-cap post-2006 per Martineau "Rest in Peace PEAD" 2021; Subrahmanyam microcap-exclusion test collapses replication t-stat 2.18 → 1.43. Surviving PEAD is microcap-only; zero overlap with v2.1+v2.2 tier definitions.
...
```bash
python3 -m src.eval.gates.envelope_shape --envelope <path-to-envelope.json>
```
...
```json
{
  ...
  "tl_dr": {
    ...
    // NOTE (retired 2026-05-24): The `tl_dr.scenarios_strategic` JSON block was deleted as part of the
    // single-case analog-as-prediction retirement (planned in /review-me v5-final, executed 2026-05-24).
    // Rationale: per Bessembinder 2018 + Green-Armstrong 2007 + Mauboussin Base Rate Book 2016, single-case
    // historical analog drawdown magnitudes are not empirically valid forecasting evidence (32% accuracy ≈
    // chance + survivorship bias). The bearish/bullish magnitude signal is carried by mechanical reference-class
    // data already present in the envelope:
    //   - cf-07 catastrophic-FAIL override (mauboussin_reverse_dcf cohort multiple threshold)
    //   - reverse_dcf implied growth + cohort mean comparison (statistical, n>1)
    // Multi-analog illustrative context lives in `report.structural_theory.detail` and `report.reasoning.detail`
    // ONLY (subject to strategic-analyst.md `historical_analogs[]` schema constraints — cap=2, mechanism-first).
    "operating_ranges": {
...
| technical_entry    | **REQUIRED**         | Optional                 | DCF-anchored |
...
**TL;DR balance**: `tl_dr.scenarios_quant` carries the cashflow-frame bear/base/bull range. Strategic content (7 Powers verdict, capital-allocation grade, multi-analog reference-class context) is rendered in the `report.structural_theory.detail` + `report.reasoning.detail` rows — NOT in a separate `tl_dr.scenarios_strategic` block. The `scenarios_strategic` field was retired 2026-05-24 (see migration note in §8 schema); single-case analog drawdown magnitudes are not empirically valid forecasting evidence (Bessembinder 2018 + Green-Armstrong 2007 + Mauboussin Base Rate Book 2016).
...
The categorical `summary_code` is DERIVED from the report content (it is not an independent decision — it is a downstream-systems-friendly compression of the Structural Theory + Technical Entry/Exit rows + sleeve_cap_check; the legacy counterfactual-veto contributor was retired 2026-05-17, see §4).
...
4. **Canonical on-disk PM Report artifact (HARD SPEC — load-bearing for output consistency).**
   ...
   **Sleeve cap status:** <PASS | PASS_SOFT_WARNING | VIOLATION | VIOLATION_DEFENSIVE_CHECK>
   ...
   ## Headline + Catalyst Probability Frame
   <(1) Bold decision headline single line: "DECISION: <BUY|HOLD|TRIM|SELL>  |  CONVICTION: <HIGH|MEDIUM|LOW>  |  SIZE BAND IF FRESH: <min%>–<max%>"; (2) Rule-engine derivation: 3–4 line summary citing kills_fired count + conviction rollup result + cell matrix verdict + catalyst_modifier_applied bps; (3) Next-catalyst probability frame: catalyst name + date + T-Nd from as_of_date (from catalyst-scout envelope catalysts[0] where confidence='high' within catalyst_scout.window.high_conviction_catalyst_days), options-market implied prior (13d IV / 90d IV / inversion / P/C ratio from catalyst-scout positioning panel), system overlay directional read (one paragraph; asymmetric to upside/downside rationale grounded in rally-into-print pattern + outside_view_divergence_pp + WS bar position), 4-bucket outcome probability table (Blowout / Modest beat / In-line-soft / Miss) with P (system judgment) and stock outcome range, footer disclosure: "Probabilities are judgment-based, not backtested. Options market is the load-bearing prior on MAGNITUDE; directional asymmetry is the system overlay on top.">

   ## System Price Anchors
   <Reference price table with columns Anchor | Price | Source. Required rows: Austere DCF bear/base/bull ranges (from quant envelope), Inherited DCF bear/base/bull ranges (from quant envelope), WS sell-side mean/high/low/median PTs (from Stage 1 yfinance pulls in main-session context), 252d high/low (from tactical or flow envelope), MA50 + MA200 (from flow envelope), current spot (from mcp__market_data real-time quote). Followed by one-paragraph distribution read: where spot sits in the anchor distribution; which anchors are above/below; implied drawdown to nearest bear-DCF zone. Reference levels ONLY — NO position-specific math (no liquidation thresholds, no take-profit ladders, no hard stops; those are operator translation work).>

   ## Panel Consensus + Disagreement Trace
   ### 2.1 Voter matrix
   <7-column table: Stage | Agent | Dim voted on | Direction | Strength | Evidence anchor | Can be overridden by. Cover 8 in-chain voters in pipeline order: quant-analyst (Stage 1, PRICE, ▼|▲|◆, HIGH|MED|LOW), strategic (Stage 1, COMPANY, …), tactical (Stage 1, TECHNICAL, …), flow (Stage 1, TECHNICAL/flow, …), CDD integration (Stage 2, SYNTHESIS, --), catalyst-scout (Stage 3, MODIFIER, LOW|MED|HIGH), pm-supervisor (Stage 4, SYNTHESIS, --), evaluator (Stage 5, PROCESS, --). Separate WS reference-voice row visually at the bottom (not weighted in synthesis; reconciliation reference only).>
   ### 2.2 Disagreement trace
   <Narrative section with 3 labeled splits: PRIMARY SPLIT (highest-strength opposite-direction voter pair; resolution stage + logic + confidence), SECONDARY SPLIT (next-strength disagreement; surfacing mechanism — typically tactical vs flow surfaced via cell matrix per Section 2.1 v5-final), CRITICAL OVERRIDE (which structural override fired — typically Overlay 2 reinvestment-vs-Helmer split or §2.6 stress-test kill — and what it constrained).>
   ### 2.3 Synthesis weighting summary
   <In-chain voice count by direction (bearish on PRICE / bullish on COMPANY / etc.) + reference-voice note (WS rating vs PT contradiction if present) + single-voter robustness check table (4 rows minimum: flip each HIGH-strength voter; show if decision changes; conclude "Decision is/is-not robust to single-voter perturbation").>

   ## Consensus Reconciliation
   <(1) WS mean target vs spot: explicit pct diff + interpretation ("rating persistence ≠ PT persistence" framing if applicable); (2) Spread of analyst PTs (high / low / median) with cluster locations + named-firm anchors at the extremes; (3) Where consensus and system actually agree (e.g., lowest analyst PT sits above/below system's bear DCF range); (4) Operational read: is WS the validating or contradicting data point relative to system decision. Pure reconciliation prose — NO new analytical claims; every numeric must trace to Stage 1 yfinance pulls.>

   ## TL;DR
   <scenarios_quant + operating_ranges + top_catalysts_90d + reevaluation_triggers — populated from §8 tl_dr block. NOTE: decision_headline moved to ## Headline + Catalyst Probability Frame above; do NOT duplicate it here.>
   ...
   <Four required sub-elements: (1) Current weakness statement (versioned-by-release static text): the system's structural bottom-blindness — trend-following technical overlays (TSMOM, MA-distance, Donchian, Antonacci) confirm bearish momentum at extremes when price is most attractive; (2) What this run did NOT see (dynamic, conditional on tier + signal state): e.g., "for a thematic_growth name in a deep drawdown with intact Helmer Powers, the system would emit HOLD throughout the inflection rather than catching the recovery"; (3) Planned improvement (current roadmap line): "v0.4 inflection gate (mean-reversion + 5-condition AND-gate) is the planned mitigation; currently in spec-research phase; calibration against counterfactual_ledger required before promotion to load-bearing"; (4) Override-discipline note: "If the operator is tempted to override this decision based on the disclosure above, remember: bottom-blindness applies to ENTRY signals, not EXIT/HOLD signals; at this spot the system's price math is unambiguous regardless of bottom-calling capability".>
...
5. **Termination criteria (HARD EXECUTION GATE — runs immediately before declaring the agent DONE; this is NOT an advisory audit log, it is the agent's own termination-blocking check).**
   ...
   **This block is the FINAL step of the agent's execution. The agent MUST NOT emit "done" or return control until ALL 11 termination checks return PASS. Failing a check on the first attempt MUST trigger an automatic re-write to the canonical path, then a re-check, BEFORE returning control. If re-write fails 3 consecutive times on the same axis, halt with structured error — do NOT silently degrade or accept the stray path.**

   **Execution order is non-optional:** (1) write file → (2) run all 11 termination checks → (3) IF every check PASSES, return done; ELSE re-write to canonical path and re-run checks; repeat up to 3 times; then halt with structured error. This is a hard execution gate, not an audit log.
   ...
   | 4 | All 14 required H2 sections present in order: **Decision Cell Matrix** (FIRST — per §7.6 mandatory top-of-report) / Headline + Catalyst Probability Frame / System Price Anchors / Panel Consensus + Disagreement Trace / Consensus Reconciliation / TL;DR / 6-Dimension Structured Report / §2.6 Adversarial Stress-Test Summary / Summary Code Derivation / System-Limitation Disclosure / Banned-Outputs Check / Audit Trail Hint / JSON Envelope (13 visible in order; H1 header + frontmatter make 14 with Decision Cell Matrix as the first H2). Decision Cell Matrix section must contain the 3×3 ASCII grid, FUND axis breakdown table, TECH axis breakdown table, cross-cell verdict line, consistency check, **a Fundamental Track block (verbatim from `decision_cell_matrix.fundamental_track` rendered inside a fenced text block, OR an "N/A" header followed by the `null_reason` narrative when null)**, **a Technical Track block (same rendering rule: verbatim fenced text when non-null, "N/A" header + `null_reason` narrative when null)**, and migration_triggers bullets per §7.6. **If `fundamental_track` AND `technical_track` are both null, `null_reason` MUST be present and non-empty (rendered once under whichever N/A header appears first).** Track block USD-literal check: when a track is non-null, it MUST contain at least one USD literal matching `\$\d` (per §7.6 v2 USD-primary rule). **Panel Consensus + Disagreement Trace section MUST contain all three H3 subsections (2.1 Voter matrix / 2.2 Disagreement trace / 2.3 Synthesis weighting summary).** **Headline + Catalyst Probability Frame section MUST contain a 4-bucket probability table with P (system judgment) + stock outcome range per bucket plus the judgment-based disclosure footer.** **System-Limitation Disclosure section MUST contain all four sub-elements (current weakness statement / what this run did NOT see / planned improvement roadmap line / override-discipline note).** | Re-emit with missing sections populated from upstream memos; matrix + tracks can be re-derived deterministically from quant/strategic/tactical/catalyst-scout envelopes per §7.6 v2 emission rules; Panel Consensus + Headline + System-Limitation sections can be re-derived from already-in-context envelope data per their template specs above |
   ...
   | 8 | JSON Envelope section contains a valid `json` fenced block matching the §8 schema                       | Re-emit JSON block                                                       |
   | 9 | Banned-Outputs Check section explicitly states pass/fail for: Stovall rotation / PEG-only / ARK point targets / Fed-without-HFI / framework-imbalance | Re-emit Banned-Outputs Check with explicit verdicts (not "N/A") |
   | 10| `execution_context.pm_report_path` in the DB INSERT (step 1) equals the relative path of the written file | Halt; the DB row and the on-disk artifact MUST agree                     |
   | 11| No additional `pm_supervisor.md`, `pm_decision.md`, or other ad-hoc PM markdown files exist in `<REPO_ROOT>/memos/`, the bg-job dir, or `/tmp` for this run | Delete strays; canonicalize on the §9 step 4 path only                  |

   The agent does NOT emit "complete" or insert the DB row until ALL 11 checks pass. If a check fails 3 times consecutively on the same axis, halt and surface a structured error to the orchestrator — do NOT silently degrade.
...
**REMINDER:** §9 step 5 is the first-class enforcement; the agent does not finish until all 11 checks pass. Non-canonical `pm_report_path` → evaluator HG-16 rejects → run blocked from emitting `execution_recommendations`.
...
**HARD MANDATE — pm-supervisor MUST NOT compute or synthesize `austere_dcf_base` from cohort base rates, Mauboussin reverse-DCF implied values, peer-set medians, or any other synthesis-layer fallback. `austere_dcf_base` is OWNED by quantitative-analyst per Decision D2 = Option α (documented in `.claude/agents/analysts/quantitative-analyst.md` §4 "Dual-DCF mandate") and `.claude/agents/eval/evaluator.md` HG-20. Synthesizing the value at pm-supervisor layer is FORBIDDEN — that bypasses the dual-DCF discipline gate (Bug 10).**
...
3. `tl_dr.scenarios_quant` MUST be populated with bear/base/bull entries. The `tl_dr.scenarios_strategic` field MUST be ABSENT — the single-case analog-as-prediction block was retired 2026-05-24 (see §8 migration note). If a future regression re-introduces the key, treat as a hard failure and strip before persisting; strategic content belongs in `report.structural_theory.detail` + `report.reasoning.detail`, not in a separate TL;DR scenarios block.
...
**Why this is load-bearing:** the Claude Code PostToolUse hook fires automatically after your Agent() return and runs the Tier-1 deterministic validator (`src.shared.agent_harness.orchestrator_step`) against the file at the canonical path. If the file is missing, the hook BLOCKS the orchestrator with a "did not persist" feedback message and your run is rejected before evaluator. If the file exists but fails validation, the hook returns a delta_prompt for targeted re-emission — see `scripts/post_agent_validate.sh` for the exit-code semantics.
