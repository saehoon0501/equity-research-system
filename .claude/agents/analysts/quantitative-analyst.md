...
- **Emit `roic_methodology_regime: 'intangibles_adjusted'` UNCONDITIONALLY for `tier ∈ {core_fundamental, thematic_growth}`. Warm-start carve-out: NONE.** Prior briefs from pre-Step-4 (before commit 3b5b027) used `regime='gaap'`; this is a stamped historical artifact and DOES NOT inherit into the current run. The 5 numeric fields and the regime flag are recomputed/re-emitted under intangibles-adjusted calculus on every dispatch where this tier rule applies. HG-38 enforces this at PostToolUse — emitting `regime='gaap'` for tier ∈ {core_fundamental, thematic_growth} triggers a delta_prompt retry, NOT a silent acceptance. Do NOT introduce auxiliary block keys (e.g. `shadow_mode_for_warm_start_label_calculus_note`) to rationalize a gaap fallback; the §3.10 schema is a closed 9-key set + the canonical skip sentinel, and unrecognized keys are surfaced by HG-38 as a soft warn audit signal.
...
Detects code/parameter pipeline drift against the validated baseline. Runs on commits touching `.claude/agents/analysts/quantitative-analyst.md` §3.10, `evaluator_gates/intangibles_adjustment_shape.py`, EPW parameter file, or `src/intangibles/**`. Test: compute MSFT FY2024 adjusted NOPAT via current code path; assert ∈ [$103B, $113B] (±5% of Mauboussin $108B published anchor, reproduced at $106.56B during Step 3 desk-calc). Fail blocks commit via pre-commit hook. Executable: `scripts/canary_intangibles_msft.sh` (follow-up commit — spec defines contract; script enforces). NOT a per-ticker validator — independent regression test against the only currently-available published reproducibility anchor.
...
- `structural_impairment_anchor` (bear case): cite a specific structural impairment — moat fade, Power lost, capital-allocation misstep, regulatory/secular shift — with a specific mechanism (Power-evidence-floor failure / capital-allocation grade collapse / quality-gate degradation / reinvestment-moat label drop) and a falsifying observable + resolution date.
...
2. Shell out to the deterministic print-date projector:
   ...
   ```bash
   python3 -m src.shared.data_layer.print_date_lookup \
     --ticker <TICKER> \
  ...
  - framework_key: damodaran_narrative_dcf  # this IS the inherited_dcf block (Bug 8 — dual-DCF mandate)
    ...
    output:
      ...
      bear_case_narrative:  # required, symmetric
        structural_impairment_anchor: <moat-fade / Power-lost / cap-allocation-misstep / regulatory-shift, with mechanism: power_loss / cap_alloc_collapse / quality_gate_fail / reinvest_moat_drop>
        distinct_arc_description: <1-2 sentences — qualitatively different from base>
        falsifying_observable: <specific measurable claim with threshold>
        falsifier_resolution_date: <YYYY-MM-DD, ≤36 months forward>
