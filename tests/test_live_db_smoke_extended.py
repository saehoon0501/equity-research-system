...
class TestLiveDbSmokeExtended:
    ...
    # ------------------------------------------------------------------
    # 2. counterfactual veto orchestrator writes
    # ------------------------------------------------------------------

    def test_live_counterfactual_retrieval_writes(
        self, live_conn_raw, hmac_keys_class, savepoint
    ):
        """End-to-end pipeline writes counterfactual_retrievals + veto_lifecycle.

        Also verifies HMAC integration: a tampered peak_pain row is dropped
        from the active retrieval pool with a system_errors entry.
        """
        ...
        from src.counterfactual_veto.retrieval import load_catalog_from_pg

        # Insert one valid SURVIVOR row + one tampered SURVIVOR row.
        valid_uc = {
            ...
            "industry_tailwind": "STRONG",
        }
        tampered_uc = dict(valid_uc)
        valid_case = f"{_RUN_TAG}-PP-VALID"
        tampered_case = f"{_RUN_TAG}-PP-TAMPER"
        _insert_peak_pain_row(
            live_conn_raw,
            case_id=valid_case, ticker="NVDA", sector="tech_saas",
            outcome="SURVIVOR",
            universal_core=valid_uc, sector_extensions={},
            hmac_key=hmac_keys_class["peak_pain"],
        )
        _insert_peak_pain_row(
            live_conn_raw,
            case_id=tampered_case, ticker="AMD", sector="tech_saas",
            outcome="SURVIVOR",
            universal_core=tampered_uc, sector_extensions={},
            hmac_key=hmac_keys_class["peak_pain"],
            tamper=True,
        )

        # Helper: run a SELECT and return list-of-dicts.
        def _query_fn(sql: str) -> list[dict[str, Any]]:
            with live_conn_raw.cursor() as cur:
                cur.execute(sql)
                cols = [d.name for d in cur.description]
                return [dict(zip(cols, r)) for r in cur.fetchall()]

        # Execute helper for system_errors write path.
        def _execute(sql: str, params: tuple) -> None:
            # Translate $1/$2 placeholders to %s for psycopg3 server-side.
            converted = sql
            i = 1
            while f"${i}" in converted:
                converted = converted.replace(f"${i}", "%s")
                i += 1
            with live_conn_raw.cursor() as cur:
                cur.execute(converted, params)

        catalog = load_catalog_from_pg(
            _query_fn,
            peak_pain_hmac_secret=hmac_keys_class["peak_pain"],
            execute=_execute,
        )
        # Tampered row is dropped from the active pool.
        case_ids = {c.case_id for c in catalog}
        assert valid_case in case_ids
        assert tampered_case not in case_ids

        # system_errors row was written for the tampered drop.
        with live_conn_raw.cursor() as cur:
            cur.execute(
                "SELECT count(*) FROM system_errors "
                "WHERE source = 'counterfactual_veto.retrieval' "
                "  AND error_type = 'peak_pain_hmac_invalid' "
                "  AND blocked_decision = %s",
                (f"counterfactual_retrieval_skip_case:{tampered_case}",),
            )
            (sys_err_count,) = cur.fetchone()
        assert sys_err_count >= 1

        # Build a candidate at 2× threshold (B_prime → 24pp; we pass 30pp).
        candidate = CandidateFeatures(
            ticker=_TEST_TICKER, sector="tech_saas",
            extraction_date="2026-04-29",
            universal_core=valid_uc,
            sector_extensions={}, consensus={},
        )

        def _premortem_lookup(ticker: str, now: _dt.datetime, lookback: int) -> bool:
            return True  # premortem present → Layer 2 satisfied

        inputs = PipelineInputs(
            ticker=_TEST_TICKER, mode="B_prime",
            candidate=candidate,
            catalog=catalog,
            fires=[],  # no fires → Layer 2 may bail; but with no fires expected
            trigger_event_at=_dt.datetime(2026, 4, 25, tzinfo=_dt.timezone.utc),
            drawdown_vs_benchmark_pp=-30.0,
            parameters_version=None,
            catalog_version_hash=f"hash-{_RUN_TAG}",
            recommendation_ref=None,
        )

        decision = run_pipeline(
            inputs,
            premortem_lookup=_premortem_lookup,
            execute=_execute,
            now=_dt.datetime(2026, 4, 29, tzinfo=_dt.timezone.utc),
        )

        # The pipeline either runs Layer 3 OR is blocked at Layer 2 by
        # 'fires=[]' (which would bypass the retrieval write). We accept
        # both routes, but assert that in any path that reaches Layer 3,
        # a counterfactual_retrievals row was written.
        with live_conn_raw.cursor() as cur:
            cur.execute(
            ...
            (ret_count,) = cur.fetchone()

        if decision.cut_status in (
            "blocked_veto_operator_override_required",
            "blocked_veto_mixed_review_required",
            "proceed_per_mode_polarity",
        ):
            assert ret_count == 1, (
                "Pipeline reached Layer 3 — counterfactual_retrievals row "
                "must be written"
            )
    ...
    # ------------------------------------------------------------------
    # 6. calibration capture: outcomes UPDATE state-table
    # ------------------------------------------------------------------

    def test_live_calibration_capture_writes(
        self, live_conn, live_conn_raw, hmac_keys_class, savepoint
    ):
        """Emit a P7 recommendation, write operator_override + outcomes,
        then UPDATE only the T+N return columns (state-table contract per
        migration 013) and verify a wider UPDATE is rejected."""
        import psycopg
        from src.p5_watchlist import add_to_watchlist
        from src.p7_recommendation_emitter import emit_recommendation

        wl_inp = _build_watchlist_input(ticker=f"{_TEST_TICKER}CAL")
        add_to_watchlist(
        ...
        # operator_override referencing the recommendation.
        with live_conn_raw.cursor() as cur:
            cur.execute(
                """
                INSERT INTO operator_overrides (
                    recommendation_id, ticker, override_date,
                    override_type, prior_value, new_value,
                    rationale, operator
                ) VALUES (
                    %s, %s, NOW(),
                    'sizing',
                    '{"initial_pct": 1.5}'::jsonb,
                    '{"initial_pct": 1.0}'::jsonb,
                    'Tightening initial size pending earnings.',
                    'operator'
                ) RETURNING override_id
                """,
                (str(outcome.recommendation_id), wl_inp.ticker),
            )
            (override_id,) = cur.fetchone()

        # recommendation_outcomes row: T+N nulls initially. We need to
        # populate benchmark_return_30d at INSERT time because the
        # state-guard trigger blocks UPDATE on benchmark_return_*
        # columns (only t_plus_*_return / close_date / last_updated_at
        # are mutable per migration 013).
        with live_conn_raw.cursor() as cur:
            cur.execute(
                """
                INSERT INTO recommendation_outcomes (
                    recommendation_id, ticker, recommendation_date,
                    benchmark, benchmark_return_30d
                ) VALUES (
                    %s, %s, CURRENT_DATE, 'SPY', 0.025
                ) RETURNING outcome_id
                """,
                (str(outcome.recommendation_id), wl_inp.ticker),
            )
            (outcome_id,) = cur.fetchone()

        # Narrow UPDATE allowed — only T+N return + close_date + last_updated_at.
        with live_conn_raw.cursor() as cur:
            cur.execute(
                """
                UPDATE recommendation_outcomes
                   SET t_plus_30d_return = 0.045,
                       t_plus_30d_close_date = CURRENT_DATE + 30,
                       last_updated_at = NOW()
                 WHERE outcome_id = %s
                """,
                (str(outcome_id),),
            ...
            cur.execute(
                "SELECT delta_vs_benchmark_30d FROM recommendation_outcomes "
                "WHERE outcome_id = %s",
                (str(outcome_id),),
            )
            (delta,) = cur.fetchone()
        assert delta is not None
        assert float(delta) == pytest.approx(0.020, abs=1e-9)

        # Wider UPDATE on benchmark column blocked by state-guard.
        with pytest.raises(psycopg.errors.RaiseException):
            with live_conn_raw.transaction():
                with live_conn_raw.cursor() as cur:
                    cur.execute(
                        "UPDATE recommendation_outcomes SET benchmark = 'QQQ' "
                        "WHERE outcome_id = %s",
                        (str(outcome_id),),
                    )

        # UPDATE on benchmark_return_* also blocked (state-table contract).
        with pytest.raises(psycopg.errors.RaiseException):
            with live_conn_raw.transaction():
                with live_conn_raw.cursor() as cur:
                    cur.execute(
                        "UPDATE recommendation_outcomes "
                        "SET benchmark_return_30d = 0.040 "
                        "WHERE outcome_id = %s",
                        (str(outcome_id),),
                    )
        ...
        with pytest.raises(psycopg.errors.RaiseException):
            with live_conn_raw.transaction():
                with live_conn_raw.cursor() as cur:
                    cur.execute(
                        "DELETE FROM recommendation_outcomes WHERE outcome_id = %s",
                        (str(outcome_id),),
                    )
