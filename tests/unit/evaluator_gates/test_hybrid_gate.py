...
from src.evaluator_gates import REGISTRY, validate_all
from src.evaluator_gates._hybrid_gate import (
    HYBRID_ESCALATE,
    HYBRID_FAIL,
    HYBRID_PASS,
    HYBRID_GATE_NAME,
...
from src.evaluator_gates import _anchor_set as anchor_mod
...
def test_unconfigured_judge_emits_warning(caplog):
    ...
    import src.evaluator_gates._hybrid_gate as hg

    hg._WARNED_UNCONFIGURED = False
    with caplog.at_level("WARNING", logger="src.evaluator_gates._hybrid_gate"):
...
def caplog_collect():
    ...
    logger = logging.getLogger("src.evaluator_gates._hybrid_gate")
    handler = _H()
