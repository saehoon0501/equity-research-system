...
**Update protocol:** when the Phase 2 trigger condition is mechanically detected (operator ad-hoc SQL per F4), commit a doc patch to this file with the exact date. The doc patch IS the gate event — there is no other event source. Operator + future-self both rely on this row being current.
...
NOT IMPLEMENTED. When the first ESCALATE fires in production:
1. Operator manually unblocks the run (existing process — verbal / git note).
2. Operator manually appends a row matching the schema above to `logs/validation_attempts.jsonl`.
3. After 1-2 manual rounds, decide whether to implement the writer (a script invoked via slash command, or a hook on operator-unblock action).

The spec exists pre-launch so the FIRST ESCALATE event is not lost to verbal-only resolution. Implementation can wait for the first production ESCALATE because ESCALATE is by construction low-frequency.
...
```
Locked construct:    <discipline | deployment | attention-floor>
...
Once locked, the tracking schema design (table additions / file naming / persistence path) flows from the chosen candidate(s). The implementation is downstream of this lock; the lock itself must precede the first 50-envelope window's close so Phase 2 ack-rate is computable.

---

## §4 — Deferred / post-launch items (recorded here for completeness)

| Item | Reason | Trigger to revisit |
|---|---|---|
| F1 — counterfactual_ledger schema add for tactical_disposition column | Only needed if JSON-scan latency proves slow at scale | Phase 2 trigger fires + scan latency > 5min |
| G3-OPEN — counterfactual_ledger.envelope_id column purpose | Possibly vestigial; no launch-blocking consequence | Quarterly housekeeping |
| F4 — Phase 2 trigger detection automation | Ad-hoc SQL at review cadence acceptable | Only if envelope cadence exceeds review cadence |
| Section 2.1 Phase 3 default action at deadline | Spec'd as "re-merge to single BUY" but reversible if evidence emerges | Phase 3 deadline approach |
