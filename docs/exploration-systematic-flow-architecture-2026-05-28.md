...
> **AMENDED 2026-05-30 (operator) — evidence generation is in-process CPCV replay, not a single live forward window.** Out-of-sample evidence for promotion is now generated by re-simulating each candidate over **purged/embargoed combinatorial cross-validation (CPCV) partitions of realized history** (via the `reactive-replay-harness` spec), NOT by deploying a live-forward paper window and waiting. Reason: the mandated overfitting diagnostic (PSR/MinTRL + DSR + PBO, §14.11 #3) needs **multiple IS/OOS partitions** that a single forward window cannot supply; walk-forward-on-deploy is also the weakest OOS validator on overfitting metrics (§14.11 #5 cross-cut). The `walk_forward_window` correlation key (LANDED, mig 048) is **retained but now labels a CPCV partition**, not a single live forward window — the key name is unchanged; only its referent is. The original single-forward-window framing below is the superseded 2026-05-29 record. (A spec/command rename of `walkforward-tuning-loop` to a CPCV-accurate name is a separate **operator decision**, deferred.)
...
- **The forward window *is* the out-of-sample test.** Promotion is earned on live/paper forward performance, not on a backtest over stale history. This is what makes walk-forward-on-deploy the antidote to overfitting rather than another route to it. **(Amended 2026-05-30: the OOS test is now the CPCV partition set, not a single forward window — see the amendment note above; promotion is earned on purged cross-validated replay, deflated by PBO/DSR over the trial set.)**
