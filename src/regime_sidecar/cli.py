...
Usage:
    ...
    python -m src.regime_sidecar.cli

    # Specific date.
    python -m src.regime_sidecar.cli --date 2026-04-29

    # Cold-start backfill: T-12mo of history, write cold_start=True.
    python -m src.regime_sidecar.cli --cold-start

    # Cold-start with custom horizon.
    python -m src.regime_sidecar.cli --cold-start --months 18

    # Dry run: print results JSON to stdout, no DB write.
    python -m src.regime_sidecar.cli --dry-run
...
from src.regime_sidecar.classifier import (
    run_daily_classification,
    to_serializable,
)
from src.regime_sidecar.persistence import backfill_cold_start, write_classifications
