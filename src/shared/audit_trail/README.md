...
src/shared/audit_trail/
  ...
  cli.py             — `python -m src.shared.audit_trail.cli` entry point
  README.md          — this file
```
...
```bash
python -m src.shared.audit_trail.cli 8f2e1234-aaaa-bbbb-cccc-dddddddddddd
```

Drill into a stage:

```bash
python -m src.shared.audit_trail.cli 8f2e1234-...  --stage stage_2_debate
```

Latest by ticker:

```bash
python -m src.shared.audit_trail.cli --latest AAPL
```

HMAC chain verify:

```bash
AUDIT_HMAC_KEY=... python -m src.shared.audit_trail.cli 8f2e1234-... --verify
```
...
|--------------------------|----------------------------------------------------------------------------|----------------------------------|-----------------------------------------|
| `AUDIT_HMAC_KEY`         | `audit_provenance` chain (Stage 1A/1B/2/3 of P3, debate, kills, etc.)      | `src/p3_mechanical_scorer/`      | `src/shared/audit_trail/hmac_verify.py`        |
...
All four producers MUST canonicalize via the helpers in `src/shared/audit_trail/hmac_verify.py` so cross-module verification works under one rule. Per remediation requirement: every audit/HMAC row written by any module verifies cleanly under the canonical scheme.
