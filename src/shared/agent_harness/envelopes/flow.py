...
Mirrors src/shared/agent_harness/envelopes/tactical.py. Schema enforced by
``src/eval/gates/flow_envelope_shape.py`` (HG-FLOW) and extended with:
...
(parity with src/overlays/flow/contracts.py::FlowSignal).
...
class FlowEnvelope:
    ...
    Parity with src/overlays/flow/contracts.py::FlowSignal — same
